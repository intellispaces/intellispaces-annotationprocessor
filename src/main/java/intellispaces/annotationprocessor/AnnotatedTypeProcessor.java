...
public abstract class AnnotatedTypeProcessor extends AbstractProcessor {
  ...
  public AnnotatedTypeProcessor(Class<? extends Annotation> annotation, Set<ElementKind> applicableKinds) {
    ...
    for (ElementKind elementKind : applicableKinds) {
      if (!ENABLED_ELEMENT_KINDS.contains(elementKind)) {
        throw new UnexpectedViolationException("Unsupported element kind {}", elementKind);
      }
    }
    ...
    for (Element annotatedElement : roundEnv.getElementsAnnotatedWith(annotation)) {
      ...
      try {
        ...
        for (ArtifactMaker maker : makers) {
          List<GeneratedArtifact> artifacts = maker.make(annotatedType);
  ...
  private void writeArtifact(GeneratedArtifact artifact) throws IOException {
    if (artifact.type() == GeneratedArtifactTypes.Java) {
      writeJavaFileArtifact(artifact.asJavaArtifact().orElseThrow());
    } else {
      throw new UnexpectedViolationException("Unsupported artifact type " + artifact.type().name());
    }
  }

  private void writeJavaFileArtifact(JavaArtifact artifact) throws IOException {
    ...
    try (var writer = new PrintWriter(file.openWriter())) {
      writer.write(artifact.javaSource());
    }
  }
